...
neighbors <- function(grid, height, width){
...
race_check <- function(grid, height, width, tolerance){
  neighborhood <- neighbors(grid, height, width)
...
schelling <- function(height = 50, width = 100, tolerence = 0.33,
  ...
  grid <- data.frame(expand.grid(height = 1:height, width = 1:width),
                     empty = FALSE,
                     satisfied = FALSE,
